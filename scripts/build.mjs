...
const assets={};for(const [name,type]of Object.entries(files))assets['/'+name]={type,body:name==='app.js'?(await fs.readFile('public/app.js','utf8')).replace(/render\(\);\s*\}\)\(\);\s*$/,await fs.readFile('public/cloud-ui.js','utf8')+'\n'+await fs.readFile('public/master-spec.js','utf8')+'\nrender();\n})();'):await fs.readFile('public/'+name,'utf8')};
